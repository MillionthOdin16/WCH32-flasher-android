...
,
            serde_yaml::from_str(include_str!("../devices/0x20-CH32F20x-Compact.
yaml"))?,
...
,
    ...
    pub fn find_chip(&self, chip_id: u8, device_type: u8) -> Result<Chip> {
        let family = self
            ...
            .ok_or_else(|| anyhow::format_err!("Device type of 0x{:02x} not foun
d", device_type))?;
...
fn parse_address_and_offset<'de, D>(deserializer: D) -> std::result::Result<u32,
 D::Error>
where
    D: serde::Deserializer<'de>,
{
    ...
    if s.starts_with("0x") || s.starts_with("0X") {
        Ok(u32::from_str_radix(&s[2..], 16).expect(&format!("error while parseri
ng {:?}", s)))
...
pub fn parse_number(s: &str) -> Option<u32> {
    if s.starts_with("0x") || s.starts_with("0X") {
        Some(u32::from_str_radix(&s[2..], 16).expect(&format!("error while parse
ring {:?}", s)))
    } else if s.starts_with("0b") || s.starts_with("0B") {
        Some(u32::from_str_radix(&s[2..], 2).expect(&format!("error while parser
ing {:?}", s)))
    } else {
