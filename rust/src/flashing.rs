...
> {
        ...
        let code_flash_protected = chip.support_code_flash_protect() && resp.pay
load()[2] != 0xa5;
        let mut btver = [0u8; 4];
    ...
    pub fn new_from_serial(port: Option<&str>, baudrate: Option<Baudrate>) -> Re
sult<Self> {
    ...
    pub fn reidenfity(&mut self) -> Result<()> {
        let identify = Command::identify(self.chip.chip_id, self.chip.device_typ
e);
        let resp = self.transport.transfer(identify)?;

        anyhow::ensure!(resp.payload()[0] == self.chip.chip_id, "chip id mismatc
h");
        anyhow::ensure!(
    ...
    pub fn unprotect(&mut self, force: bool) -> Result<()> {
        ...
        let write_conf = Command::write_config(CFG_MASK_RDPR_USER_DATA_WPR, conf
ig);
    ...
    pub fn flash(&mut self, raw: &[u8]) -> Result<()> {
        ...
        let key_checksum = key.iter().fold(0_u8, |acc, &x| acc.overflowing_add(x
).0);
        ...
        anyhow::ensure!(resp.payload()[0] == key_checksum, "isp_key checksum fai
led");
    ...
    pub fn write_eeprom(&mut self, raw: &[u8]) -> Result<()> {
        let key = self.xor_key();
        // let key_checksum = key.iter().fold(0_u8, |acc, &x| acc.overflowing_ad
d(x).0);
        ...
        let resp = self.transport.transfer(isp_key)?;
        anyhow::ensure!(resp.is_ok(), "isp_key failed");
        // anyhow::ensure!(resp.payload()[0] == key_checksum, "isp_key checksum
failed");
...
;
    ...
    pub fn dump_eeprom(&mut self) -> Result<Vec<u8>> {
        ...
        while address < self.chip.eeprom_size as u32 {
            let chunk_size = u16::min(CHUNK as u16, self.chip.eeprom_size as u16
 - address as u16);
            ...
            if resp.payload()[2..] == [0xfe, 0x00] {
                anyhow::bail!("EEPROM read failed, required chunk size cannot be
 satisfied");
            }
...
()> {
    ...
    fn write_data_chunk(&mut self, address: u32, raw: &[u8], key: [u8; 8]) -> Re
sult<()> {
        let xored = raw.iter().enumerate().map(|(i, x)| x ^ key[i % 8]);
        let padding = rand::random();
    ...
    pub fn dump_config(&mut self) -> Result<()> {
        ...
        for reg_def in &self.chip.config_registers {
            ...
            for field_def in &reg_def.fields {
                let bit_width = (field_def.bit_range[0] - field_def.bit_range[1]
) as u32 + 1;
                let b = (n >> field_def.bit_range[1]) & (2_u32.pow(bit_width) -
1);
                println!(
                    "  {:<7} {} 0x{:X} (0b{:b})",
                    format!("[{:}:{:}]", field_def.bit_range[0], field_def.bit_r
ange[1]),
                    field_def.name,
                    b,
                    b
