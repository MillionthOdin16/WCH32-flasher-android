...
use anyhow::Result;
use object::{
    elf::FileHeader32, elf::PT_LOAD, read::elf::FileHeader, read::elf::ProgramHe
ader, Endianness,
...
pub fn objcopy_binary(elf_data: &[u8]) -> Result<Vec<u8>> {
    ...
    let binary = object::read::elf::ElfFile::<FileHeader32<Endianness>>::parse(e
lf_data)?;
    ...
    // Ref: https://docs.oracle.com/cd/E19683-01/816-1386/chapter6-83432/index.h
tml
    for segment in elf_header.program_headers(elf_header.endian()?, elf_data)? {
        // Get the physical address of the segment. The data will be programmed
to that location.
        ...
        let segment_data = segment
            ...
            .map_err(|_| anyhow::format_err!("Failed to access data for an ELF s
egment."))?;
        if !segment_data.is_empty() && segment.p_type(endian) == PT_LOAD {
            log::info!(
                    "Found loadable segment, physical address: {:#010x}, virtual
 address: {:#010x}, flags: {:#x}",
                    p_paddr,
                    p_vaddr,
                    flags
                );
            ...
            for section in binary.sections() {
                let (section_offset, section_filesize) = match section.file_rang
e() {
                    Some(range) => range,
                    None => continue,
                };
                ...
                if segment_offset <= section_offset
                    && segment_offset + segment_filesize >= section_offset + sec
tion_filesize
                {
                    log::debug!(
                    ...
                    for (offset, relocation) in section.relocations() {
                        log::debug!("Relocation: offset={}, relocation={:?}", of
fset, relocation);
                    }
                    section_names.push(section.name()?.to_owned());
                }
            }
            let section_data = &elf_data[segment_offset as usize..][..segment_fi
lesize as usize];
...
fn merge_sections(mut sections: Vec<(u32, Cow<[u8]>)>) -> Result<Vec<u8>> {
    ...
    let end_address = sections.last().unwrap().0 + sections.last().unwrap().1.le
n() as u32;
