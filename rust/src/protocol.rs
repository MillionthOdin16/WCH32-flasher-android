...
impl Command {
    ...
    pub fn into_raw(self) -> Result<Vec<u8>> {
        match self {
            ...
            Command::IspEnd { reason } => Ok([commands::ISP_END, 0x01, 00, reaso
n].to_vec()),
            Command::IspKey { key } => {
            ...
            // 08 00 00 00
            Command::Erase { sectors } => {
                let mut buf = [commands::ERASE, 0x04, 0x00, 0x00, 0x00, 0x00, 0x
00];
...
/// Response to a Command. The request cmd type is ommitted from the type defini
tion.
...
impl fmt::Debug for Response {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Response::Err(code, data) => write!(f, "ERROR({:x})[{}]", code, hex:
:encode(data)),
        }
    }
}
