...
impl ValueEnum for Baudrate {
    ...
    fn to_possible_value(&self) -> Option<PossibleValue> {
        match self {
            Baudrate::Baud115200 => Some(PossibleValue::new("Baud115200").aliase
s(["115200"])),
            Baudrate::Baud1m => {
                Some(PossibleValue::new("Baud1m").aliases(["1000000", "1_000_000
", "1m"]))
            }
            Baudrate::Baud2m => {
                Some(PossibleValue::new("Baud2m").aliases(["2000000", "2_000_000
", "2m"]))
            }
        }
    }
    ...
    pub fn set_baudrate(&mut self, baudrate: impl Into<u32>) -> Result<()> {
        ...
        if baudrate != self.serial_port.baud_rate()? {
            ...
            if let Some(0xfe) = resp.payload().first() {
                log::info!("Custom baudrate not supported by the current chip. U
sing 115200");
            } else {
...
impl Transport for SerialTransport {
    fn send_raw(&mut self, raw: &[u8]) -> Result<()> {
        ...
        v.extend_from_slice(&[raw.iter().fold(0u8, |acc, &val| acc.wrapping_add(
val))]); // Append the CRC
    ...
    fn recv_raw(&mut self, _timeout: Duration) -> Result<Vec<u8>> {
        ...
        let payload_data_len = payload_head_buf.pread_with::<u16>(2, scroll::LE)
? as usize;
        ...
        // Read the checksum and verify against actual sum calculated from
        // entire payload (header + data).
        let checksum = resp_vec.iter().fold(0u8, |acc, &val| acc.wrapping_add(va
l));
        anyhow::ensure!(
